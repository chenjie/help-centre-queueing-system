...
int check_overflow(int cur_length, char *sen_buf) {
    if (strlen(sen_buf) + cur_length >= OUT_BUF_SIZE) {
        return 1;
    } else {
        return 0;
    }
...
char *print_currently_serving(Ta *ta_list) {
    char *res_buf = malloc(OUT_BUF_SIZE);
    res_buf[0] = '\0';
    int cur_length = 0;
    if (ta_list == NULL) {
        strcat(res_buf, "No TAs are currently working.\r\n");
        return res_buf;
    }
    char sen_buf[OUT_BUF_SIZE];
    while (ta_list != NULL) {
       if (ta_list->current_student != NULL) {
           sprintf(sen_buf, "TA: %s is serving %s.\r\n",
                ta_list->name,
                ta_list->current_student->name);
       } else {
           sprintf(sen_buf, "TA: %s has no student\r\n", ta_list->name);
       }
       if (check_overflow(cur_length, sen_buf) == 1) {
           strncat(res_buf, sen_buf, OUT_BUF_SIZE-cur_length-1);
           return res_buf;
       }
       strcat(res_buf, sen_buf);
       cur_length += strlen(sen_buf);
...
char *print_full_queue(Student *stu_list) {
    char *res_buf = malloc(OUT_BUF_SIZE);
    res_buf[0] = '\0';
    strcat(res_buf, "Full Queue\r\n");
    int cur_length = strlen("Full Queue\r\n");
    char sen_buf[OUT_BUF_SIZE];
    while (stu_list != NULL) {
        sprintf(sen_buf, "Student %s:%s\r\n",stu_list->name, stu_list->course->code);
        if (check_overflow(cur_length, sen_buf) == 1) {
            strncat(res_buf, sen_buf, OUT_BUF_SIZE-cur_length-1);
            return res_buf;
        }
